...
Public Module SseBodyGenIniWriter
    ...
    ''' previa verificada. La garantía —y por qué no puede ser atómica
    ''' bajo MO2/Vortex— vive en UN solo lugar, la cabecera de
    ''' <c>BSA_BA2_Library_DLL.EscrituraEnElLugar</c>. No se repite acá.</para>
    ''' <para>⛔ Y los DOS archivos son un PAR: se escriben por
    ''' <see cref="EscrituraDelParBodyGen.EscribirElPar"/>, que deshace el primero si falla el segundo.</para></summary>
    Public Sub Emit(dataPath As String, targetPluginBaseName As String, entries As List(Of NpcEntry))
        ...
        ' ⛔ LOS DOS JUNTOS O NINGUNO — la MISMA ley que el escritor de FO4 y por la misma puerta, para que no
        ' se arreglen por separado. Ver `EscrituraDelParBodyGen`.
        EscrituraDelParBodyGen.EscribirElPar(templatesPath, Utf8SinBom(sbTemplates.ToString()),
                                             morphsPath, Utf8SinBom(sbMorphs.ToString()))
    End Sub

    ''' <summary>Los bytes del .ini: UTF-8 SIN BOM — el parser de skee no lo espera.</summary>
    Private Function Utf8SinBom(texto As String) As Byte()
        Return New UTF8Encoding(encoderShouldEmitUTF8Identifier:=False).GetBytes(texto)
    End Function
