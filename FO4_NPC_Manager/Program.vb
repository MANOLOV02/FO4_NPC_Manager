...
Module Program
    ...
    Private Sub RealMain(args As String())
        ...
        If args IsNot Nothing AndAlso args.Any(Function(a) String.Equals(a, "--tree-diag", StringComparison.OrdinalIgnoreCase)) Then
            RunTreeDiag(args)
            Return
        End If
        ...
        ' puede exhibir ningún corpus. Ver memoria 00-reglas-self-tests-no-van-en-el-binario.
...
#End If
    ...
    ' ============================================================================================
    ' ⛔ INSTRUMENTO TEMPORAL (tree-diag) — BORRAR ENTERO CUANDO TERMINE LA MEDICION DEL ARBOL
    ' ============================================================================================

    ''' <summary><c>--tree-diag</c>: arranca la app SIN interaccion, mide desde el arranque del proceso
    ''' hasta que el arbol de NPC esta poblado, mide despues el costo de repoblarlo con varios filtros, y
    ''' sale. Es la unica forma de tener el numero del arbol sin manejar la GUI a mano.
    '''
    ''' <para>NO reimplementa la carga: abre el Preflight de verdad fuera de pantalla y le dispara el OK,
    ''' asi que la secuencia que se mide es EXACTAMENTE la de produccion —misma resolucion de config, mismo
    ''' orden de carga, mismo diccionario, mismos sidecars—. Duplicar esa ley para poder medirla habria
    ''' medido otra cosa.</para>
    '''
    ''' <para>Las ventanas se muestran DE VERDAD, en -32000: un TreeView sin handle no paga el costo de
    ''' insercion en Win32, que es justo lo que se quiere medir. Ver DesignerCostProbe, donde una sonda que
    ''' usaba CreateControl() sin mostrar el form informo numeros que no existian.</para></summary>
    Private Sub RunTreeDiag(args As String())
        EnsureConsole()
        Dim arranqueProceso = Process.GetCurrentProcess().StartTime
        ...
        Console.WriteLine("== tree-diag ==")
        Console.WriteLine($"   juego: {Config_App.Current.Game}")

        ' ---- 1. Preflight, fuera de pantalla, aceptado por codigo ----
        Dim swPre = Stopwatch.StartNew()
        Dim pre As New Preflight_Form()
        pre.StartPosition = FormStartPosition.Manual
        pre.Location = New Point(-32000, -32000)
        pre.ShowInTaskbar = False
        pre.Show()
        Application.DoEvents()
        Dim hallados = pre.Controls.Find("ButtonOk", searchAllChildren:=True)
        If hallados Is Nothing OrElse hallados.Length = 0 Then
            Console.Error.WriteLine("FATAL: no encontre ButtonOk en el Preflight.")
            Environment.ExitCode = 1
            Return
        End If
        DirectCast(hallados(0), Button).PerformClick()
        ' El handler del OK es Async: hay que BOMBEAR mensajes hasta que termine, no bloquear el hilo.
        Dim limite = TimeSpan.FromMinutes(10)
        While pre.DialogResult <> DialogResult.OK AndAlso swPre.Elapsed < limite
            Application.DoEvents()
            Threading.Thread.Sleep(15)
        End While
        swPre.Stop()
        If pre.DialogResult <> DialogResult.OK Then
            Console.Error.WriteLine($"FATAL: el Preflight no termino en {limite.TotalMinutes} min (DialogResult={pre.DialogResult}).")
            Environment.ExitCode = 1
            Return
        End If
        Console.WriteLine($"   preflight (config + orden de carga + plugins + archives): {swPre.ElapsedMilliseconds} ms")

        ' ---- 2. MainForm, fuera de pantalla, hasta que el arbol este ----
        Dim swMain = Stopwatch.StartNew()
        Dim listo = False
        Dim msCtor As Long = 0, msShow As Long = 0, msHastaCarga As Long = 0
        Dim mf As New MainForm(pre.LoadedPluginManager, pre.LoadedDataPath,
                               pre.LoadedAutoGenPlugins, pre.LoadedSidecars)
        msCtor = swMain.ElapsedMilliseconds
        mf.ArbolListo = Sub() listo = True
        mf.CargaArranca = Sub() msHastaCarga = swMain.ElapsedMilliseconds
        mf.StartPosition = FormStartPosition.Manual
        mf.Location = New Point(-32000, -32000)
        mf.ShowInTaskbar = False
        mf.Show()
        msShow = swMain.ElapsedMilliseconds
        While Not listo AndAlso swMain.Elapsed < limite
            Application.DoEvents()
            Threading.Thread.Sleep(15)
        End While
        swMain.Stop()
        swTotal.Stop()
        If Not listo Then
            Console.Error.WriteLine("FATAL: el arbol no termino de poblarse.")
            Environment.ExitCode = 1
            Return
        End If

        Console.WriteLine($"   MainForm: constructor                     : {msCtor} ms")
        Console.WriteLine($"   MainForm: Show() (dispara Load)           : {msShow - msCtor} ms")
        Console.WriteLine($"   MainForm: Load -> arranca LoadDataAsync   : {msHastaCarga - msShow} ms")
        Console.WriteLine($"   MainForm: LoadDataAsync (parseo + arbol)  : {swMain.ElapsedMilliseconds - msHastaCarga} ms")
        Console.WriteLine($"   MainForm hasta el arbol poblado           : {swMain.ElapsedMilliseconds} ms")
        Console.WriteLine($"   desglose de la app                        : {mf.TiemposDeCargaTexto}")
        Console.WriteLine($"   NPC en la lista                           : {mf.CuantosNpc}")
        Console.WriteLine($"   TOTAL desde el arranque del proceso       : {(Date.Now - arranqueProceso).TotalMilliseconds:F0} ms")
        Console.WriteLine($"   TOTAL desde el inicio de RunTreeDiag      : {swTotal.ElapsedMilliseconds} ms")

        ' ---- 3. Repoblados: lo que el usuario siente en CADA tecla ----
        Console.WriteLine("")
        Console.WriteLine("   repoblado del arbol (mediana de 5, en ms):")
        Dim casos = New String()() {
            New String() {"sin filtro", ""},
            New String() {"una letra a", "a"},
            New String() {"raider", "raider"},
            New String() {"sin resultados zzzz", "zzzz"},
            New String() {"la letra p", "p"}}
        For Each caso In casos
            Dim medidas As New List(Of Long)
            For i = 1 To 5
                medidas.Add(mf.MedirRepoblado(caso(1)))
                Application.DoEvents()
            Next
            medidas.Sort()
            Console.WriteLine($"     {caso(0),-24} {medidas(medidas.Count \ 2),6}   (min {medidas(0)}, max {medidas(medidas.Count - 1)})")
            Console.WriteLine($"       {mf.DetalleRepoblado}")
        Next

        Console.WriteLine("")
        Console.WriteLine("[tree-diag] listo.")
