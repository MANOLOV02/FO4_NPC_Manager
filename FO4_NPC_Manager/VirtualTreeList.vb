...
Public Class VirtualTreeList
    ...
    ''' <summary>Las filas seleccionadas. El control las gobierna —incluido el rango con Shift— porque
    ''' es mecanica del control, no del formulario: sin esto cada consumidor reimplementa ancla, rango y
    ''' alternado, y los tres salen distintos.</summary>
    Private ReadOnly _seleccionadas As New HashSet(Of FilaDeArbol)()

    ''' <summary>Desde donde mide el rango un Shift+click: la ultima fila elegida SIN Shift. Es lo que
    ''' hace que Shift extienda en vez de empezar de cero.</summary>
    ...
    ''' desde el dibujo de cada fila convierte un repintado en O(n²). Para el pintado están
    ''' <see cref="EstaEnLaSeleccion"/> y <see cref="CantidadSeleccionada"/>, que son O(1).</para></summary>
    Public ReadOnly Property Seleccionadas As IReadOnlyList(Of FilaDeArbol)
        Get
            Return Modelo.Visibles.Where(Function(f) _seleccionadas.Contains(f)).ToList()
        End Get
    End Property

    ''' <summary>Cuántas filas hay seleccionadas. O(1).</summary>
    Public ReadOnly Property CantidadSeleccionada As Integer
        Get
            Return _seleccionadas.Count
        End Get
    End Property

    Public Function EstaEnLaSeleccion(fila As FilaDeArbol) As Boolean
        Return fila IsNot Nothing AndAlso _seleccionadas.Contains(fila)
    End Function


    Public Sub LimpiarSeleccion()
        If _seleccionadas.Count = 0 Then Return
    ...
    Private Sub AplicarClick(fila As FilaDeArbol, modificadores As Keys)
        ...
        If conShift AndAlso _ancla IsNot Nothing Then
            Dim a = Modelo.IndiceVisible(_ancla)
            Dim b2 = Modelo.IndiceVisible(fila)
            If a >= 0 AndAlso b2 >= 0 Then
                If Not conCtrl Then _seleccionadas.Clear()
                For i = Math.Min(a, b2) To Math.Max(a, b2)
                    _seleccionadas.Add(Modelo.Visibles(i))
                Next
            End If
        ElseIf conCtrl Then
            If Not _seleccionadas.Remove(fila) Then _seleccionadas.Add(fila)
            _ancla = fila
        Else
            ...
            If _seleccionadas.Count = 1 AndAlso _seleccionadas.Contains(fila) Then Return
            _seleccionadas.Clear()
            _seleccionadas.Add(fila)
            _ancla = fila
        End If
        Invalidate()
        RaiseEvent SeleccionCambiada(Me, EventArgs.Empty)
    ...
    Public Sub Refrescar()
        Dim claveEnfocada As String = FilaEnfocadaActual()?.Clave
        _modelo.Aplanar()
        AplicarTamanio(claveEnfocada)
    End Sub
    ...
    Private Sub AplicarTamanio(claveEnfocada As String)
        BeginUpdate()
        Try
            SelectedIndices.Clear()
            VirtualListSize = _modelo.Visibles.Count
            If claveEnfocada IsNot Nothing Then
                Dim idx = _modelo.IndiceVisible(_modelo.PorClave(claveEnfocada))
                If idx >= 0 Then SelectedIndices.Add(idx)
            End If
        Finally
            EndUpdate()
        End Try
    ...
    Public Sub AlternarFila(fila As FilaDeArbol)
        ...
        Dim claveEnfocada = FilaEnfocadaActual()?.Clave
        _modelo.Aplanar()
        AplicarTamanio(claveEnfocada)
    End Sub
    ...
    Protected Overrides Sub OnDrawItem(e As DrawListViewItemEventArgs)
        ...
        Dim seleccionada = _seleccionadas.Contains(fila)
    ...
    Protected Overrides Sub OnMouseDown(e As MouseEventArgs)
        ...
        If fila IsNot Nothing AndAlso e.Button = MouseButtons.Left AndAlso EsElGlifo(fila, e.Location) Then
            ' Click en el glifo: expande y NO mueve la selección, igual que un TreeView.
            AlternarFila(fila)
            Return
        End If
    ...
    Protected Overrides Sub OnMouseUp(e As MouseEventArgs)
        ...
        If e.Button = MouseButtons.Left OrElse Not _seleccionadas.Contains(fila) Then
    ...
    Protected Overrides Sub OnSelectedIndexChanged(e As EventArgs)
        ...
        If (ModifierKeys And (Keys.Control Or Keys.Shift)) = 0 Then
            If Not (_seleccionadas.Count = 1 AndAlso _seleccionadas.Contains(fila)) Then
                _seleccionadas.Clear()
                _seleccionadas.Add(fila)
                _ancla = fila
                Invalidate()
