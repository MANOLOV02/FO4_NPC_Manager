...
persist:
    ...
    Private Sub FindHkxScan(substr As String)
        ...
        For Each k In keys
            ...
            Try
                ...
                For Each o In sg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeleton.NombreDeClase)
                ...
                Next
                For Each o In sg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbCharacterStringData.NombreDeClase)
    ...
    Private Sub HkxBoneDump(hkxPath As String, boneSubstr As String)
        ...
        Dim skel = g.EsqueletoDeAnimacion()
...
#End Region
        ...
        For Each rec In pm.GetRecordsOfType("RACE")
            ...
            For Each bk In behKeys
                ...
                Try
                    ...
                    For Each o In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbClipGenerator.NombreDeClase)
                    ...
                    Next
                    ' ⛔ CADA BLOQUE CON SU CLASE. Esto barria `BGSGamebryoSequenceGenerator` y lo leia
                    ' con `HkObj_HkbBehaviorReferenceGenerator`: dos clases HERMANAS, ninguna deriva de
                    ' la otra. Salia bien por casualidad —`BGSGamebryoSequenceGenerator.pSequence` y
                    ' `hkbBehaviorReferenceGenerator.behaviorName` estan los dos en +0x88 en FO4— y este
                    ' archivo no tiene `Option Strict`, asi que nada lo cazaba. Si Bethesda mueve uno de
                    ' los dos, esto empieza a leer basura sin fallar.
                    For Each o In Havok.Canon.Objects.HkObj_BGSGamebryoSequenceGenerator.Todos(g)
                        Dim gn = If(o.PSequence, "")
    ...
    Private Sub NifDumpRun(path As String)
        ...
        For Each blk In nif.Blocks
            ...
            Dim fl = nn.Flags_ui
            Dim nas = ""
            If (fl And HkxPoseImportSession.BitDeNoAnimSync.X) <> 0 Then nas &= "X"
            If (fl And HkxPoseImportSession.BitDeNoAnimSync.Y) <> 0 Then nas &= "Y"
            If (fl And HkxPoseImportSession.BitDeNoAnimSync.Z) <> 0 Then nas &= "Z"
            If (fl And HkxPoseImportSession.BitDeNoAnimSync.S) <> 0 Then nas &= "S"
    ...
    Private Sub BlendHintScanRun(filter As String)
        ...
        For Each k In keys
            ...
            Try
                ...
                Dim bindings = g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaAnimationBinding.NombreDeClase).ToList()
    ...
    Private Sub AnimSyncCheck(spec As String)
        ...
        Dim skel = sg.EsqueletoDeAnimacion()
        ...
        For t = 0 To anim.Animacion.NumberOfTransformTracks - 1
            ...
            ' ⛔ EL SELECTOR POR COMPONENTE ES `HkxPoseImportSession.LocalDelFrame`, NO UNA COPIA.
            ' Los siete bits de la mascara y los huecos del `hkQsTransform` viven en un solo lugar.
            clipLocal(nm) = HkxPoseImportSession.LocalDelFrame(ht, mskT, refp)
            maskOf(nm) = (((mskT And HkxPoseImportSession.BitDeTrack.TX) <> 0),
                          ((mskT And HkxPoseImportSession.BitDeTrack.TY) <> 0),
                          ((mskT And HkxPoseImportSession.BitDeTrack.TZ) <> 0),
                          ((mskT And HkxPoseImportSession.BitDeTrack.Rotacion) <> 0),
                          ((mskT And (HkxPoseImportSession.BitDeTrack.SX Or HkxPoseImportSession.BitDeTrack.SY Or HkxPoseImportSession.BitDeTrack.SZ)) <> 0))
        Next
        ...
        Dim buildLocal = Function(nm As String, honor As Boolean) As Transform_Class
                             ...
                             Dim nasM = HkxPoseImportSession.MascaraNoAnimSync(fl)
                             Dim fx = (nasM And 1) <> 0, fy = (nasM And 2) <> 0, fz = (nasM And 4) <> 0, fs = (nasM And 8) <> 0
    ...
    Private Sub ClipBaseDump(spec As String)
        ...
        Dim embSkels = cg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeleton.NombreDeClase).Select(Function(o) Havok.Canon.Objects.HkObj_HkaSkeleton.Read(cg, o)).
        ...
        For t = 0 To anim.Animacion.NumberOfTransformTracks - 1
            ...
            ' ⛔ EL SELECTOR POR COMPONENTE ES `HkxPoseImportSession.LocalDelFrame`, NO UNA COPIA.
            ' Los siete bits de la mascara y los huecos del `hkQsTransform` viven en un solo lugar.
            Dim clip0 = HkxPoseImportSession.LocalDelFrame(ht, mskT, refp)
            Dim mask = $"T:{If(((mskT And HkxPoseImportSession.BitDeTrack.TX) <> 0), "x", "-")}{If(((mskT And HkxPoseImportSession.BitDeTrack.TY) <> 0), "y", "-")}{If(((mskT And HkxPoseImportSession.BitDeTrack.TZ) <> 0), "z", "-")} R:{If(((mskT And HkxPoseImportSession.BitDeTrack.Rotacion) <> 0), "a", "-")} S:{If(((mskT And HkxPoseImportSession.BitDeTrack.SX) <> 0) OrElse ((mskT And HkxPoseImportSession.BitDeTrack.SY) <> 0) OrElse ((mskT And HkxPoseImportSession.BitDeTrack.SZ) <> 0), "a", "-")}"
    ...
    Private Sub DumpBehaviorClips(path As String)
        ...
        For Each o In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaAnimationBinding.NombreDeClase)
        ...
        Next
        Dim cgs = g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbClipGenerator.NombreDeClase).Select(Function(o) Havok.Canon.Objects.HkObj_HkbClipGenerator.Read(g, o)).Where(Function(c) c IsNot Nothing).ToList()
        Dim refs = g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbBehaviorReferenceGenerator.NombreDeClase).Select(Function(o) If(Havok.Canon.Objects.HkObj_HkbBehaviorReferenceGenerator.Read(g, o)?.BehaviorName, "")).Where(Function(s) Not String.IsNullOrEmpty(s)).Distinct().ToList()
        ...
        For Each cls In {Havok.Canon.Objects.HkObj_DynamicAnimationTaggingGenerator.NombreDeClase, Havok.Canon.Objects.HkObj_BSiStateTaggingGenerator.NombreDeClase, Havok.Canon.Objects.HkObj_HkbManualSelectorGenerator.NombreDeClase,
                         Havok.Canon.Objects.HkObj_HkbBlenderGenerator.NombreDeClase, Havok.Canon.Objects.HkObj_HkbLayerGenerator.NombreDeClase, Havok.Canon.Objects.HkObj_HkbLayer.NombreDeClase}
        ...
        For Each o In g.Objects
            ...
            Next
        Next
        For Each o In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbCharacterStringData.NombreDeClase)
    ...
    Private Sub CollectBehaviorRefs(behFile As String, referenced As HashSet(Of String), visited As HashSet(Of String), depth As Integer)
        ...
        Try
            ...
            For Each refObj In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbBehaviorReferenceGenerator.NombreDeClase)
    ...
    Private Sub CollectClipAnims(behFile As String, loader As Func(Of String, Byte()), graphCache As Dictionary(Of String, HkxObjectGraph_Class),
        ...
        For Each o In graph.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbClipGenerator.NombreDeClase)
        ...
        Next
        Dim behRoot = ActorRootC(behFile)
        For Each ro In graph.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbBehaviorReferenceGenerator.NombreDeClase)
    ...
    Private Sub CompareNifHkxBoneSets(label As String, hkxPath As String, nifPath As String)
        ...
        Dim allSk = sg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeleton.NombreDeClase).Select(Function(o) Havok.Canon.Objects.HkObj_HkaSkeleton.Read(sg, o)).
    ...
    Private Sub ComposeAndCompareSkeleton(label As String, hkxPath As String, nifPath As String)
        ...
        If clothBlk IsNot Nothing Then
            ...
            If HkxPackfileParser_Class.TryParse(clothBlk, cpf) Then
                ' ⛔ LA LEY DEL GRAFO, Y UN SOLO GRAFO. Acá se elegía el PRIMER BLOQUE y, peor, se
                ' construía el grafo DOS VECES: el objeto salía de una instancia distinta de la que
                ' había enumerado los bloques.
                Dim csk = HkxObjectGraphParser_Class.BuildGraph(cpf).EsqueletoDeAnimacion()
    ...
    Private Sub CompareLogLiveSkelVsHkx(logPath As String, hkxPath As String)
        ...
        Dim sg = HkxObjectGraphParser_Class.BuildGraph(HkxPackfileParser_Class.Parse(hbx))
        Dim skel = sg.EsqueletoDeAnimacion()
        ' ⛔ LA GUARDA QUE FALTABA. Los otros trece call sites de `EsqueletoDeAnimacion()` la tienen;
        ' este no, y `EsqueletosUsables()` ahora exige ademas que `referencePose` cubra `bones`, asi
        ' que el Nothing es alcanzable con archivos que antes pasaban.
        If skel Is Nothing OrElse skel.Bones Is Nothing Then Console.WriteLine("  hkx sin esqueleto de animacion usable") : Return
    ...
    Private Sub MountValidateRun()
        ...
        ' ⛔ LA LEY DEL GRAFO: sale de `hkaAnimationContainer.skeletons`, no del orden de los bloques.
        Dim skel = sg.EsqueletoDeAnimacion()
    ...
    Private Function CheckSkeletonFile(skelPath As String) As (String, Boolean)
        ...
        Try
            Dim g = HkxObjectGraphParser_Class.BuildGraph(HkxPackfileParser_Class.Parse(b))
            Dim skels = g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeleton.NombreDeClase).Select(Function(o) Havok.Canon.Objects.HkObj_HkaSkeleton.Read(g, o)).
            ...
            Dim nonRag = skels.Where(Function(s) Not HkxObjectGraph_Class.EsRagdoll(s.Name)).Count()
    ...
    Private Sub DumpHkxInventory(label As String, bytes As Byte())
        ...
        Try
            ...
            Next
            For Each o In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeleton.NombreDeClase)
            ...
            Next
            For Each o In g.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkaSkeletonMapper.NombreDeClase)
    ...
    Private Sub RaceAnimDiagnostics(havokSkelPath As String, renderSkelNif As String, clips As List(Of ResolvedAnimationClip))
        ...
        Else
            Try
                Dim sg = HkxObjectGraphParser_Class.BuildGraph(HkxPackfileParser_Class.Parse(skelBytes))
                ' ⛔ EL DE ANIMACION, NO EL PRIMERO DECLARADO. Un `skeleton.hkx` de behavior declara
                ' DOS — animacion y ragdoll — asi que `EsqueletoPrincipal()` aca era la moneda al aire
                ' que el doc de `EsqueletoDeAnimacion` describe.
                Dim skel = sg.EsqueletoDeAnimacion()
                If skel Is Nothing Then Throw New InvalidDataException("sin esqueleto de animacion")
    ...
    Private Sub RawClipPathDump(rb As ResolvedRaceBehavior, loader As Func(Of String, Byte()))
        ...
        If projBytes IsNot Nothing Then
            Try
                ...
                Dim charFiles As New List(Of String)
                For Each o In pg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbProjectStringData.NombreDeClase)
                ...
                For Each cf In charFiles
                    ...
                    For Each co In cg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbCharacterStringData.NombreDeClase)
        ...
        For Each bf In rb.DistinctBehaviorFiles()
            ...
            Try
                ...
                Dim gens = bg.GetObjectsByClassName(Havok.Canon.Objects.HkObj_HkbClipGenerator.NombreDeClase).ToList()
